...
class pageBuilder
{
	...
	[
		...
		"og:url" => "https://avalanche.alanbloxxr.xyz",
...
<html lang="en">
	<head>
		...
		<meta name="avalanche-csrf" content="<?=SESSION?SESSION["csrfToken"]:'false'?>"> 
...
<?php } } ?>
	...
	?>
		...
		<script>
		  ...
		  $.ajaxSetup(
		  {
			headers: { 'x-avalanche-csrf': $('meta[name="avalanche-csrf"]').attr('content') }
		  });
		...
		<script>
			...
			var avalanche_friends = 
			  ...
			  function registerFriendActionHandler()
			  {
				  ...
				  {
				    avalanche_friends.friendAction($(this).attr("aria-userid"), $(this).attr("aria-controls")).done(function(data)
				    {
				    	if(data.success)
				    	{
			  ...
			  function displayFriends(page)
			  {
			  	...
			  	avalanche_friends.getFriends(userId, page, limit).done(function(data)
			  	{ 
			  		if(data.success)
			  		{ 
			  ...
			  {
			  	avalanche_friends.getFriendRequests(page).done(function(data)
			  	{ 
			  		if(data.success)
			  		{ 
