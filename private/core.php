...
require __DIR__ . '/vendors/Parsedown.php';
...
try {
    $pdo = new PDO(
        'mysql:host='.SITE_CONFIG["database"]["host"].';dbname='.SITE_CONFIG["database"]["schema"],
    ...
    die("DB Connection Error: " . $e->getMessage() . " (Code: " . $e->getCode() . ")");
}
...
class api {
    ...
    static function requireLogin() {
        ...
        if(!isset($_SERVER['HTTP_X_avalanche_CSRF'])){ self::respond(400, false, "CSRF token not set"); }
        if($_SERVER['HTTP_X_avalanche_CSRF'] != SESSION["csrfToken"]){ self::respond(400, false, "Invalid CSRF token"); }
    }
}

class general {
    static function time_elapsed($datetime, $full = false, $ending = true) {
        if($datetime == "@"){ return "-"; }
        $now = new DateTime;
    ...
    static function filterText($text, $htmlspecialchars = true, $highlight = true) {
        ...
        return str_ireplace(["fuck", "shit", "bitch", "nigger", "nigger", "faggot", "nigga", "nibba", "nibber", "dick", "cock", "penis", "sex", "cum", "nya"], $filtertext, $text);
    }
...
class users {
    ...
    static function getUserAvatar($userId) {
        return "https://alanbloxxr.xyz/renderserver/renders/jobId512-420x420.png";
    }
    ...
    static function getForumPostCount($userId) {
        global $pdo;
        $query = $pdo->prepare("SELECT (SELECT COUNT(*) FROM avalanche.forum_threads WHERE author = :id AND NOT deleted) + (SELECT COUNT(*) FROM avalanche.forum_replies WHERE author = :id) AS totalPosts");
    ...
    static function updatePing() {
        ...
        $userId = SESSION["userId"];
        $sessionkey = $_COOKIE['avalanche_session'];
    ...
    static function updateCurrencyStipend() {
        ...
        if(!SESSION){ return false; }
        $userId = SESSION["userId"];
        if(SESSION["nextCurrencyStipend"] > time()){ return true; }
...
class session {
    static function createSession($userId) {
        ...
        $query->execute();
        if($query->fetchColumn()){ goto keygen; }
        ...
        $create->execute();
        setcookie("avalanche_session", $sessionkey, time()+(157700000*3), "/");
    }
    ...
    static function invalidateSession($sesskey) {
        setcookie("avalanche_session", $sesskey, 1, "/");
        header("Location: /");
        exit;
    }
...
if(isset($_COOKIE['avalanche_session'])) {
    $session = session::getSessionData($_COOKIE['avalanche_session']);
    if($session) {
